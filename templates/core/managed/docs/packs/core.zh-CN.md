...
若曾运行 `init-ai add python-quality`，可能多出 `pyproject.toml`、`.venv`、Ruff/Pyright/pre-commit 配置。清理后重新拉 core：

```bash
rm -rf .venv pyproject.toml uv.lock ruff.toml pyrightconfig.json pyrightconfig.pre-push.json .pre-commit-config.yaml
init-ai --update --apply
# 勿再运行 python-quality / ci-quality / mlops-gpu
```
