...
- `.gitlab-ci.yml` 和 `.gitlab/`：GitLab CI 入口与 pipeline 模板（质量检查 + GPU Runner 训练调度）。
- `Dockerfile` 和 `.devcontainer/`：可复用的 GPU 开发/训练容器模板。
...
- 复制 Ruff、Pyright、pre-commit、GitHub/GitLab CI、Docker、devcontainer 和 PR 模板。
...
更新模式会保留 `MEMORY.md`、`.cursor/project-context/`、`.cursor/lessons-learned/` 和目标项目已有的 `train.py`。它只更新模板管理的内容，例如 `.cursor/rules/*.mdc`、`CLAUDE.md`、`.cursorrules`、Ruff、Pyright、pre-commit、GitHub/GitLab workflow、Dockerfile 和 devcontainer 文件。
...
### 场景七：MLOps / GPU Runner 项目

模板现在包含一套最小 GPU 训练脚手架：

- `Dockerfile`：基于 PyTorch CUDA 的开发/训练镜像，并预装 `uv`。
- `.devcontainer/devcontainer.json`：给 Cursor / VS Code 使用的 devcontainer，复用同一个 Dockerfile，开启 GPU，并把 `/mnt/nfs_data` 挂载到 `/data`。
- `.gitlab-ci.yml`：根目录 CI 入口，只负责 include `.gitlab/ci/pipeline.yml`。
- `.gitlab/ci/pipeline.yml`：GitLab 质量检查。
- `.gitlab/ci/train.yml`：给带 `gpu` 标签的 self-hosted GitLab Runner 使用的训练任务。
- `train.py`：最小 smoke test，用来验证 PyTorch、CUDA 可见性和数据挂载。

训练任务默认比较保守：它会在 `$HOME/mlops-runs` 下创建持久运行目录，把 GitLab checkout 的项目复制过去，停止同分支上一次训练容器，然后用 detached Docker 容器启动训练。你可以在 GitLab 页面手动点 Play，也可以在 commit message 里加入 `[run train]` 自动触发。

常用 GitLab CI/CD 变量如下，可以按项目或按 Runner 覆盖：
...
真实项目中通常会把 `TRAIN_COMMAND` 改成自己的入口，例如 `uv run python -m src.train`。如果你不想构建项目 Dockerfile，而是直接使用已有镜像，可以设置 `BUILD_MLOPS_IMAGE=false`，并把 `MLOPS_DOCKER_IMAGE` 改成 `pytorch/pytorch:2.2.1-cuda12.1-cudnn8-devel` 或你的私有镜像。

`WANDB_API_KEY`、`MLFLOW_TRACKING_URI` 等密钥应放在 GitLab CI/CD Variables 里。不要把凭证写进这个仓库、Dockerfile、`.env` 或项目记忆文件。

#### GitLab Runner 部署步骤

每台 GPU 服务器需要安装 Docker、NVIDIA Container Toolkit 和 self-hosted GitLab Runner。Runner 建议使用 shell executor，并注册 `gpu` 标签。先验证服务器能运行 GPU 容器：

```bash
docker run --rm --gpus all nvidia/cuda:12.1.1-base-ubuntu22.04 nvidia-smi
```

Runner 所在用户必须能执行 Docker，并且能读取数据挂载目录：
...
如果有多台 GPU 服务器，建议使用更细的标签，例如 `gpu-a100-1`、`gpu-4090-1`，再在目标项目的 `.gitlab/ci/train.yml` 中修改 `tags`。如果所有 Runner 都叫 `gpu`，GitLab 每次只会把一个训练 job 分配给其中一台可用 Runner，不会同时跑到所有服务器上。

#### devcontainer 设置建议

建议设置 devcontainer，但把它当作“交互式开发入口”，不要把它当成训练调度的唯一方式。典型流程是：
...
然后在 Cursor 里打开这个远程目录，选择 **Reopen in Container**。在此之前，请确认服务器上存在 `/mnt/nfs_data`；如果你的数据盘路径不同，请先改目标项目里的 `.devcontainer/devcontainer.json`。

使用 devcontainer 前，请在 Cursor 中安装 **Remote - SSH** 和 **Dev Containers** 扩展。

首次验收步骤见 [MLOPS-CHECKLIST.zh-CN.md](MLOPS-CHECKLIST.zh-CN.md)。
...
- `.gitlab-ci.yml`、`.gitlab/ci/pipeline.yml` 和 `.gitlab/ci/train.yml`：GitLab 质量检查和 GPU Runner 训练调度。
- `MLOPS-CHECKLIST.zh-CN.md`：Docker、GitLab Runner、devcontainer 首次验收清单。
- `Dockerfile` 和 `.devcontainer/devcontainer.json`：共享 GPU 开发/训练环境。
