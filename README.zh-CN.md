...
- `docs/`：补充文档，含 MLOps 验收 checklist。
...
模板包含最小 GPU 训练脚手架：`Dockerfile`、`.devcontainer/`、GitLab CI（`.gitlab-ci.yml`、`.gitlab/ci/`）和 `train.py`。训练任务会派发到带 `gpu-server` 标签的 self-hosted Runner，可在 GitLab 页面手动 Play，或在 commit message 中加入 `[run train]` 触发。

部署、验收、Runner 注册、Docker 冒烟测试和 devcontainer 步骤见 [docs/MLOPS-CHECKLIST.zh-CN.md](docs/MLOPS-CHECKLIST.zh-CN.md)。
...
- `.gitlab-ci.yml`、`.gitlab/ci/quality.yml` 和 `.gitlab/ci/train.yml`：GitLab 质量检查和 GPU Runner 训练调度。
- `docs/`：补充文档，含 MLOps 验收 checklist。
